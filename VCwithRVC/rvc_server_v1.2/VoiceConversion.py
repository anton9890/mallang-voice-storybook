import json,os,sys,torch,glob,librosa
now_dir = os.getcwd()
...
if torch.cuda.is_available():
    ...
    if (
        ...
        is_half = False
        print("is_half is False")
    else:
        print("is_half is True")
...
def loadCJson(Originame):
    ...
    # octav = data['octav']
    return gender, id, age, infer_rate, #octav
...
# def setOctav(characterOct, audio_file_path):
#     average_octave = calculate_average_octave(audio_file_path)
#     octav = (characterOct - average_octave) * 14
#     print(average_octave)
#     print(octav)
#     return int(octav)
...
def getParameter(characterId,childAge,childGender,wavpath):
    characterPath = getPath(f"character/{characterId}/json", ".json")
    # characterGender, characterId, characterAge, inferRate, cOctav= loadCJson(characterPath[0])
    ...
    # f0up_key = setOctav(cOctav, input_path)
...
def seperateVoice(audio_path):
    mixwav_sc, sr = sf.read(audio_path)
    # mixwav_sc = mixwav_sc[:,1]
